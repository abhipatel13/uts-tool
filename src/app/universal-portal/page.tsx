...
import { TaskHazard, User } from "@/types"
...
export default function UniversalPortal() {
  ...
  const fetchTaskHazards = useCallback(async () => {
    try {
      ...
      const response = await TaskHazardApi.getAllByCompany(selectedCompanyFilter);
      ...
      try {
        const fallbackResponse = await TaskHazardApi.getAll();
  ...
  // const fetchSites = async (companyId: number) => {
  //   try {
  //     const response = await UniversalUserApi.getSites(companyId);
  //     if (response.status) {
  ...
  const openEditDialog = (user: User) => {
    ...
    //   email: user.email || '',
    //   role: user.role || 'user',
    //   company_id: user.company?.id || user.company_id || 0,
    //   phone: user.phone || '',
          ...
          <DialogContent>
            ...
            <form onSubmit={handleEditUser} className="space-y-4">
                ...
                <Label htmlFor="edit-company">Company</Label>
                  ...
                  onValueChange={(value) => setEditFormData({...editFormData, company_id: parseInt(value)})}
                  >
                    <SelectTrigger>
                    <SelectValue placeholder="Select company" />
                    </SelectTrigger>
