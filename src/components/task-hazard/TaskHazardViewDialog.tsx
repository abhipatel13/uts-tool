...
import { MapPin, Calendar, AlertTriangle, Users, ShieldCheck, Lock, GraduationCap, CircleDot } from "lucide-react"

interface TaskHazardViewDialogProps {
  open: boolean;
...
export function TaskHazardViewDialog({
  open,
  onOpenChange,
  task,
  onEdit,
}: TaskHazardViewDialogProps) {
  if (!task) return null;

  const rejectionComments = task.latestApproval?.status === 'rejected' 
    ? task.latestApproval?.comments?.split('Comments: ').at(-1) || "No comments"
    : null;

  const riskCount = task.risks?.length || 0;
  const highRiskCount = task.risks?.filter(r => r.requiresSupervisorSignature).length || 0;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full max-w-4xl mx-auto h-screen sm:h-auto sm:max-h-[90vh] overflow-y-auto p-0 gap-0">
        <VisuallyHidden.Root>
          <DialogTitle>Task Hazard Assessment Details</DialogTitle>
        </VisuallyHidden.Root>
        ...
        <div className="px-6 pt-6 pb-4 border-b bg-gray-50/50">
          <div className="flex items-start justify-between gap-4 pr-8">
            <div className="flex-1 min-w-0">
              ...
              <h2 className="text-lg font-semibold text-gray-900 leading-tight">
                {task.scopeOfWork || "Untitled Assessment"}
              </h2>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3 text-sm text-gray-600">
            <div className="flex items-center gap-1.5">
              ...
              <span className="truncate max-w-[200px]">{task.location || "No location"}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Calendar className="h-3.5 w-3.5 text-gray-400" />
              <span>{task.date} {task.time && `at ${task.time}`}</span>
            </div>
            {riskCount > 0 && (
              <div className="flex items-center gap-1.5">
        ...
        <div className="px-6 py-5 space-y-5">
              ...
              <div className="space-y-2 min-w-max">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <IndividualsList individuals={task.individuals} />
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900 whitespace-nowrap">{task.supervisor || "Not assigned"}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2">
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-gray-900 truncate">{task.assetSystem || "Not specified"}</p>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <MapPin className="h-4 w-4 text-gray-400 mt-0.5 flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="text-xs text-gray-500">Geo Fence Limit</p>
                    <p className="text-sm text-gray-900">{task.geoFenceLimit || 200} Feet</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            <RequirementBadge 
              ...
              label="System Lockout"
              value={task.systemLockoutRequired}
            />
            <RequirementBadge 
              icon={<GraduationCap className="h-3.5 w-3.5" />}
              label="Trained Workforce"
              value={task.trainedWorkforce}
            />
          </div>
